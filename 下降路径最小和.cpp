...
//在这段代码中，我们使用动态规划来解决最小下降路径和的问题。具体的步骤如下：
//1. 创建一个大小为 n x n 的二维数组 dp，并将其初始化为最大值，用于存储每个位置的最小下降路径和。
//2. 初始化 dp 的第一行为 matrix 的第一行，表示从第一行开始的路径和。
//3. 从第二行开始，遍历每个位置 (i, j)，计算当前位置的最小下降路径和。我们选择上一行相邻位置的最小路径和，并将其加上当前位置的值。
//4. 在遍历完成后，我们在最后一行中找到最小的路径和作为结果。
//5. 返回最小的路径和作为最终结果。
